<?php
  ...
  *
  * $Id: phpagi.php,v 1.4 2006/11/01 12:03:22 leob Exp $
  *
  ...
  */
  class AGI
  {
    ...
    * Start MusicOnHold.
    *
    * @link http://bugs.digium.com/view.php?id=3703&nbn=5
    * @param string $class
    * @return array, see evaluate for return information.
    */
    function exec_start_musiconhold($class='')
    {
      // NOTE: added this function in 2006.08.07 (LB)
      return $this->exec('StartMusicOnHold', $class);
    }
    ...
    * @return array, see evaluate for return information.
    */
    function exec_stop_musiconhold()
    {
      // NOTE: added this function in 2006.08.07 (LB)
      return $this->exec('StopMusicOnHold');
    }
    ...
    */
    ...
    function sable2wav($text, $escape_digits='', $frequency=8000)
    {
      $text = trim($text);
      if($text == '') return true;
      ...
      $hash = md5($text);
      $fname = $this->config['phpagi']['tempdir'] . DIRECTORY_SEPARATOR;
      $fname .= 'sable2wav_' . $hash;

      // create wave file
      if(!file_exists("$fname.wav"))
      {
        // write text file
        if(!file_exists("$fname.sable"))
        {
          $fp = fopen("$fname.sable", 'w');
          $sable_text_header = <<<EOD
<?xml version="1.0"?>
...
<SABLE>
EOD;
          $sable_text_footer = <<<EOD
</SABLE>          
EOD;
          fputs($fp, $sable_text_header . $text . $sable_text_footer);
          fclose($fp);
        }

        shell_exec("{$this->config['festival']['text2wave']} -F $frequency -o $fname.wav $fname.sable");
      }
      else
      {
        touch("$fname.sable");
